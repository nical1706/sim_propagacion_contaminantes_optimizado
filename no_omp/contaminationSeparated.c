...
Tensor* get_P_whole(SimulationData* sim_data, MapData* map_data, Tensor* G, int* cartypes, char** pollutants) {
    ...
    for (int t = 0; t < times; t++) {
        for (int k = 0; k < num_tensors; k++) {
            ...
            //unsigned long long t_0 = __rdtsc();
            //Bucle difusión
            for (int i = 1; i <= rows; i++) {
                for (int j = 1; j <= cols; j++) {
                    size_t id = IDX_2D(i, j, cols_l);
    ...
    //Para evadir el uso de condicionales en el código que impidan la implementación de SIMD se ha añadido la iteración t=times para que
